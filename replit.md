...
- **RBAC**: Tenant-scoped role-based access control enforces permissions at form edit, override activation, workflow execution, workflow approval, and change approval. Permissions are evaluated via centralized `authorize(ctx, userId, permission, resourceType?, resourceId?)` in `rbacService.ts`. Deny-before-allow policy evaluation. SystemContext bypasses RBAC. Default roles: Admin (all permissions), Editor (form.view, form.edit, workflow.execute, override.activate), Viewer (form.view). User identity passed via `x-user-id` header. No inline permission checks outside authorize().

## RBAC System (server/services/rbacService.ts)
- **Models**: RbacPermission (global), RbacRole (tenant-scoped), RbacRolePermission (role→permission), RbacUserRole (user→role), RbacPolicy (tenant-scoped, role→resourceType→effect).
...
- **Evaluation**: authorize() resolves user roles (active, tenant-scoped) → role permissions → policies. Fails closed if no explicit allow. SystemContext bypasses.
...
- **RBAC Admin Protection**: All mutating RBAC routes (role create/disable/enable, role-permission management, user-role assignment, policy CRUD) are gated by requireRbacAdmin() which checks change.approve permission (Admin-only). Read-only routes (GET permissions, GET roles) remain open. Disabled roles do not grant permissions (filtered in authorize()).
...
- **API Routes**: GET/POST /api/rbac/permissions, GET/POST /api/rbac/roles, POST /api/rbac/roles/:id/disable, POST /api/rbac/roles/:id/enable, GET/POST /api/rbac/roles/:id/permissions, DELETE /api/rbac/roles/:id/permissions/:permissionId, GET/POST /api/rbac/users/:userId/roles, DELETE /api/rbac/users/:userId/roles/:roleId, GET/POST /api/rbac/policies, DELETE /api/rbac/policies/:id, POST /api/rbac/seed-defaults.
