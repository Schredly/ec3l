...
- **Runner Service Boundary**: The runner is extracted into a standalone `runner/` package (peer to `server/`), containing all execution logic: types, adapters (local/remote), boundary guard, boundary errors, telemetry emitter, runner service, adapter factory, and an HTTP server. Shared execution context types (`TenantContext`, `ModuleExecutionContext`, `Capability`, `CapabilityProfileName`) live in `shared/executionTypes.ts` and are imported by both `runner/` and `server/`. The control plane (`server/`) communicates with the runner via in-process function calls (default) or HTTP (remote mode) through `server/execution/` which is a thin shim re-exporting from `runner/`. The runner can be started independently as a standalone HTTP service via `runner/main.ts` (entry point) or in-process via `initRunner({ telemetrySink })`. A `runner/Dockerfile` enables containerized deployment. Environment config: `RUNNER_ADAPTER=local|remote`, `RUNNER_URL` (default `http://localhost:4001`), `RUNNER_PORT` (default `4001`), `RUNNER_TIMEOUT_MS` (default `30000`).
- **Runner HTTP API**: Standalone runner exposes `POST /execute` (accepts `ExecutionRequest`, returns `ExecutionResult`) and `GET /health`. The server uses Node.js `http` module (no Express dependency). Telemetry emits from the runner side (console sink when standalone, storage-backed sink when in-process). Key file: `runner/server.ts`.
- **Runner Execution Interface**: Explicit control-plane → runner boundary for all execution paths, ensuring no filesystem or shell access exists outside the runner interface. Uses a configurable adapter pattern (LocalRunnerAdapter for in-process, RemoteRunnerAdapter for HTTP) with standardized `ExecutionRequest` and `ExecutionResult` objects. RemoteRunnerAdapter sends HTTP POST to the runner service URL with configurable timeout. Key files: `runner/types.ts`, `runner/adapters/localRunnerAdapter.ts`, `runner/adapters/remoteRunnerAdapter.ts`, `runner/adapterFactory.ts`.
